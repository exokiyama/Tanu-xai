...
export interface CommandDefinition { name: string; aliases?: string[]; category: Category; description: string; usage?: string; permissions?: Permission[]; groupOnly?: boolean; ownerOnly?: boolean; adminOnly?: boolean; cooldown?: number; handler: (ctx: CommandContext) => Promise<void>; }
export interface BotSettings { name: string; prefix: string; mode: 'public' | 'private'; watermark: string; packname: string; author: string; menustyle: 'classic' | 'minimal' | 'premium'; }
