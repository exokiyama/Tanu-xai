...
const command = {
  ...
  async handler(sock, message, args, context) {
    ...
    const { listChats } = require('../utils/chat-registry.js');
    const chats = listChats().filter(chat => chat.jid && chat.jid !== 'status@broadcast');
    if (chats.length === 0) return reply('📢 No chats have been observed by this bot process yet.');
    ...
    for (const chat of limitedChats) {
      try {
        await sock.sendMessage(chat.jid, { text: broadcastMessage });
        successCount++;
      } catch (error) {
        failCount++;
        console.error('[BROADCAST] Failed to send to', chat.jid, error.message);
      }
