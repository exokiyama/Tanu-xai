...
const command = {
  ...
  async handler(sock, message, args, context) {
    ...
    const key = (eq >= 0 ? raw.slice(0, eq) : args[0]).trim().toUpperCase();
    const value = (eq >= 0 ? raw.slice(eq + 1) : args.slice(1).join(' ')).trim();

    if (!key || !value) return reply('❌ Usage: .setvar <KEY> <value> or .setvar KEY=value');
    ...
    // Protection scopes are persisted by the protection manager.
    if (['ANTI_DELETE', 'ANTI_EDIT', 'ANTI_VV'].includes(key)) {
      const { updateProtection } = require('../utils/protection.js');
      const protectionKey = key === 'ANTI_VV' ? 'antivv' : key === 'ANTI_EDIT' ? 'antiedit' : 'antidelete';
      const scope = String(value).trim().toLowerCase();
      const enabled = scope !== 'off';
      await updateProtection(protectionKey, { enabled, scope: enabled ? scope : 'off' });
      return reply(`✅ ${key} set to ${scope}.`);
    }
