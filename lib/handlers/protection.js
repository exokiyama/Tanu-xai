import { isProtectionEnabled, getProtection, updateProtection } from '../utils/protection.js';
import { phoneFromJid } from '../../config/config.js';
 ...
 */
export async function handleMessagesUpdate(sock, updates) {
 ...
 */
export async function handleMessagesUpsert(sock, messages) {
 ...
 */
export async function handleCall(sock, callData) {
 ...
 */
export async function handlePMProtection(sock, message) {
 ...
 */
export async function resetWarns(sock, chatId, sender) {
  const key = `${chatId}:${sender}`;
  warnCount.delete(key);
