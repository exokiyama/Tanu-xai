...
function generateMenu(commands, style = 'normal', isOwner = false) {
  ...
  switch (style) {
    ...
    case 'image-link':
      return generateImageLinkMenu(grouped, isOwner);
    case 'normal':
    default:
...
function generateImageLinkMenu(grouped, isOwner) {
  const normal = generateNormalMenu(grouped, isOwner);
  const buttons = [];
  if (config.repositoryUrl) buttons.push({ type: 'url', url: config.repositoryUrl, text: '📦 Repository' });
  if (config.websiteUrl) buttons.push({ type: 'url', url: config.websiteUrl, text: '🌐 Website' });
  return { type: 'image-button', text: normal.text, buttons, image: { url: config.MENU_IMAGE_URL || null, caption: normal.text } };
}
