...
function formatMessage(msg) {
  ...
  const sender = msg.senderName || msg.senderJid?.split('@')[0] || 'Unknown';
  const location = msg.groupId ? `👥 ${msg.groupName || 'Group'}` : '💬 Private';
  let body;
  switch (msg.messageType) {
    case 'text': body = msg.content || '(empty text)'; break;
    case 'image': body = `📷 Image${msg.caption ? ` — ${msg.caption}` : ''}`; break;
    case 'video': body = `🎥 Video${msg.caption ? ` — ${msg.caption}` : ''}`; break;
    case 'audio': body = '🎵 Audio'; break;
    case 'sticker': body = '✨ Sticker'; break;
    case 'document': body = `📄 Document${msg.content ? ` — ${msg.content}` : ''}`; break;
    case 'location': body = `📍 Location${msg.content ? ` — ${msg.content}` : ''}`; break;
    case 'contact': body = `👤 Contact${msg.content ? ` — ${msg.content}` : ''}`; break;
    case 'command': body = `⚙️ ${msg.content || ''}`; break;
    default: body = `📦 ${msg.messageType || 'Unknown'}`;
  }
  return `┌─ ${time} ─────────────────\n│ ${sender}\n│ ${location}\n│ ${body}\n└────────────────────────`;
}
