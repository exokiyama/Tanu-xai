const { getCategories, getKeyMetadata } = require('./config-manager.js');
/**
 ...
 */
function redactValue(value, isSensitive) {
  if (!isSensitive) {
 ...
 */
function generateSettingsDisplay(configWithMetadata) {
 ...
 * @returns {string} Formatted text
 */
function generateSingleSettingDisplay(key, metadata) {
  if (!metadata) {
 ...
 * @returns {string} Formatted text
 */
function generateCategoryDisplay(categoryKey, categoryData) {
  const categories = getCategories();
