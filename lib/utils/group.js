 ...
 */
export function getUserFromContext(message, text) {
 ...
 */
export function getUsersFromContext(message, text) {
  // Check if message is a reply
  if (message.quoted && message.quoted.sender) {
    return [message.quoted.sender];
  }
 ...
 */
export function isBotAdmin(participants, botJid) {
 ...
 */
export async function tagAllMembers(sock, chatId, participants, message = '') {
 ...
 */
export async function getGroupMetadata(sock, chatId) {
  try {
 ...
 */
export function validateUsersForAction(users, participants, excludeAdmins = true) {
  const valid = [];
  const invalid = [];
  const alreadyAdmin = [];
  const notInGroup = [];
