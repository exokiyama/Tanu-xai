 ...
 */
export function checkRateLimit(userId, action, maxRequests = 10, windowSeconds = 60) {
 ...
 */
export function getRateLimitStatus(userId, action, maxRequests = 10, windowSeconds = 60) {
  const key = `${userId}:${action}`;
  const now = Date.now();
  const windowMs = windowSeconds * 1000;
 ...
 */
export function formatRateLimitMessage(result) {
