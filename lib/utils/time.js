 ...
 */
function getTimestamp(inSeconds = false) {
 ...
 */
function getTimezoneFromOffset(offsetHours) {
...
function parseDuration(value) {
  ...
  if (!raw) return null;
  const match = raw.match(/^(\d+(?:\.\d+)?)\s*(s|sec|secs|m|min|mins|h|hr|hrs|d|day|days|w|week|weeks)$/);
  if (!match) return null;
  const n = Number(match[1]);
  const unit = match[2];
  const factor = unit.startsWith('s') ? 1000 : unit.startsWith('m') ? 60000 : unit.startsWith('h') ? 3600000 : unit.startsWith('d') ? 86400000 : 604800000;
